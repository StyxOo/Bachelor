...
function myCallback() {
    const randCountry = Math.random().toString(16).substr(2, 8); // 6de5ccda
    const randRefugees = +Math.random().toString().substr(2, 8);
    const newLine = new Object({'country' : randCountry, 'refugees' : randRefugees})
    globalData.push(newLine)
    render(globalData)
}
